...
export function DisabilityPill({ defaultName, onDelete, onUpdate, ...rest }: DisabilityPillProps) {
	...
	return (
		...
		>
			...
			{isEditing ? (
				<Button onClick={() => setIsEditing(false)} className="p-0 border-0 h-6 w-6" color="danger">
					<svg
				...
				<Button onClick={() => setIsEditing(true)} className="p-0 border-0 h-6 w-6" color="danger">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
			...
			<Button onClick={onDelete} className="p-0 border-0 h-6 w-6" color="danger">
				<svg
					aria-hidden="true"
