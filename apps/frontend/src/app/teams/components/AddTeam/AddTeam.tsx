...
import { CreateTeamInputSchema } from "@shared/common/schemas";
import { zodResolver } from "@hookform/resolvers/zod";
import type { CreateTeamInput } from "@shared/common/types";
...
export function AddTeam() {
	...
	const { register, handleSubmit, formState } = useForm<Omit<CreateTeamInput, "photo">>({
	...
	async function onSubmit(data: Omit<CreateTeamInput, "photo">) {
		const formData = new FormData();
		formData.append("name", data.name);
		if (uploadedFiles) {
			formData.append("photo", uploadedFiles[0]);
		}
		const { status, error } = await createTeam(formData);
	...
	return (
		<>
			<AddButton onClick={() => setModalOpen(true)} subject="Team" />
			<Modal
				isOpen={modalOpen}
			...
			>
				<div className="flex justify-between">
					<Title order={5}>Add a Team</Title>
				...
				>
					<TextInput
						label="Name"
						placeholder="Enter a team name (eg. 'XJC Northwest')"
						disabled={isPending}
						{...register("name")}
